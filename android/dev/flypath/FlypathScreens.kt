...
public class FlypathScreenStackView(context: Context) : ViewGroup(context) {
  ...
  private fun drive(
    ...
    animator.addUpdateListener { value ->
      onFrame(value.animatedValue as Float)
    }
  ...
  private fun animateAway(screen: FlypathScreenView) {
    val shiftFrom = screen.translationX
    val shiftTo =
      if (edge == BackEventCompat.EDGE_LEFT) width.toFloat()
      else -width.toFloat()
    drive(
      POP_DURATION,
      { progress ->
  ...
  private fun refresh() {
    ...
    for ((at, screen) in screens.withIndex()) {
      ...
      val revealed =
        at == top ||
          (at == top - 1 && (swiping || running != null || screens[top].modal))
      screen.visibility = if (revealed) VISIBLE else INVISIBLE
    }

    back.isEnabled =
      active && screens.size > 1 && (screens.lastOrNull()?.gesture ?: false)
  }
}
...
public class FlypathScreenManager : ViewGroupManager<FlypathScreenView>() {
  ...
  ): FlypathScreenView = FlypathScreenView(reactContext)

  override fun updateProperties(
    view: FlypathScreenView,
    props: ReactStylesDiffMap,
  ) {
    super.updateProperties(view, props)
    val values = props.toMap()
    (values["screenKey"] as? String)?.let { value -> view.screenKey = value }
    (values["presentation"] as? String)?.let { value ->
      view.modal = value == "modal"
    }
...
public class FlypathScreenStackManager :
  ViewGroupManager<FlypathScreenStackView>() {

  override fun getName(): String = STACK_NAME

  override fun createViewInstance(
    reactContext: ThemedReactContext
  ): FlypathScreenStackView = FlypathScreenStackView(reactContext)

  override fun updateProperties(
    view: FlypathScreenStackView,
    props: ReactStylesDiffMap,
  ) {
    super.updateProperties(view, props)
  ...
  override fun getChildCount(parent: FlypathScreenStackView): Int =
    parent.screenCount
