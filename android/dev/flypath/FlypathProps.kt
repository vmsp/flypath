...
public class FlypathProps(private val values: Map<String, Any?>) {
  ...
  public fun number(name: String): Double =
    (values[name] as? Number)?.toDouble() ?: 0.0

  public fun string(name: String): String = values[name] as? String ?: ""

  public fun bytes(name: String): ByteArray =
    values[name] as? ByteArray ?: ByteArray(0)

  public fun list(name: String): List<Any?> =
    values[name] as? List<Any?> ?: emptyList()
  ...
  public companion object {
    ...
    public fun scalar(value: Any?): FlypathProps =
      FlypathProps(mapOf("value" to value))
  }
}
