...
public value class FlypathValue(private val ref: Long) {
  ...
  public fun at(index: Int): FlypathValue =
    FlypathValue(FlypathAbi.at(ref, index))

  public fun field(name: String): FlypathValue =
    FlypathValue(FlypathAbi.field(ref, name))

  public fun <T> list(decode: (FlypathValue) -> T): List<T> =
    (0 until count).map { decode(at(it)) }

  public fun <T> orNull(decode: (FlypathValue) -> T): T? =
    if (isNull) null else decode(this)
}
...
public value class FlypathOut(private val ref: Long) {
  ...
  public fun array(count: Int): FlypathOut =
    FlypathOut(FlypathAbi.outArray(ref, count))

  public fun element(index: Int): FlypathOut =
    FlypathOut(FlypathAbi.outElement(ref, index))

  public fun obj(): FlypathOut = FlypathOut(FlypathAbi.outObject(ref))

  public fun field(name: String): FlypathOut =
    FlypathOut(FlypathAbi.outField(ref, name))

  public fun <T> setList(values: List<T>, encode: (T, FlypathOut) -> Unit) {
    val target = array(values.size)
    values.forEachIndexed { index, value ->
      encode(value, target.element(index))
    }
  }
